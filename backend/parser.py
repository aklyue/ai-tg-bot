...
from bot.config import KNOWLEDGE_SOURCES, SOURCES_META
...
async def save_to_db(url: str, text: str):
    ...
    project_tag = get_project_tag(meta.get("name", "") + " " + url)

    metadata = {"source": url, "project": project_tag}
    ...
    try:
        async with async_playwright() as p:
                        ...
                        async with session.get(
                            ...
                            if response.status == 200:
                                ...
                                for script in soup(["script", "style"]):
                                    script.decompose()
                                fallback_content = soup.get_text(
                                    separator="\n", strip=True
                                )
                                if len(fallback_content) > len(content):
                                    ...
                                    log_parsing(
                                        url,
                                        "INFO",
                                        f"Fallback BeautifulSoup: {len(content)} символов",
                                    )
                except ImportError:
            ...
            log_parsing(
                url,
                "INFO",
                f"Найдено {len(internal_pages)} внутренних ссылок. Парсим первые 10...",
            )
            for link in all_links:
                if base_domain in link and not link.endswith(
                    (".pdf", ".zip", ".jpg", ".png", "#")
                ):
...
async def parse_document(url: str):
    ...
    try:
        if "disk.yandex.ru" in url:
            ...
            async with aiohttp.ClientSession() as session:
                async with session.get(
                    ...
                    if response.status != 200:
                        log_parsing(
                            url, "ERROR", f"Yandex Disk API HTTP {response.status}"
                        )
                        return False
                            ...
                            if file_url:
                                try:
                                    ...
                                    log_parsing(
                                        url,
                                        "WARNING",
                                        f"Ошибка чтения {file_name}: {e}",
                                    )

                text = (
                    "\n\n".join(text_parts)
                    if text_parts
                    else data.get("name", "Yandex Disk")
                )
                enriched = (
                    f"Информация из документа: {meta['name']}. Ссылка: {url}\n\n{text}"
                )
                await save_to_db(url, enriched)
                return True
        ...
        if "docs.google.com" in url:
            ...
            async with aiohttp.ClientSession() as session:
                async with session.get(
                    export_url, timeout=aiohttp.ClientTimeout(total=30)
                ) as response:
                    if response.status == 200:
